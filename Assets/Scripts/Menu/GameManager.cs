...
public class GameManager : MonoBehaviour
{
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        if (scene.name == nameScene && nameScene != "Menu")
        {
    ...
    private void Awake()
    {
        ...
        // HasKey kiểm tra key có tồn tại chưa, tránh ghi lại nhiều lần.
        if (!PlayerPrefs.HasKey("KeyRound"))
        {
            PlayerPrefs.SetInt("KeyRound", keyRound);
        }
        else
        {
            keyRound = (1 == PlayerPrefs.GetInt("KeyRound")) ? 1 : PlayerPrefs.GetInt("KeyRound");
        }
